...
const StorageItem = z.object({
  ...
  每件份数: z.coerce.number().transform(value => Math.max(1, Math.floor(value))).prefault(1),
  当前剩余份数: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(1),
...
const CookedItem = StorageItem.extend({
  搭配判定: MatchJudgement.prefault('无冲突'),
});
...
const TavernReputation = z.object({
  ...
  名称: z.enum(['无人知晓', '略有耳闻', '小有名气', '远近闻名', '声名远扬']).prefault('无人知晓'),
...
}, z.string().regex(/^\d{2}:\d{2}$/).or(z.literal('')));
...
const TemporaryState = z.object({
  ...
  剩余回合: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
  描述: z.string().prefault(''),
  来源物品: z.string().prefault(''),
});

const Facility = z.object({
  状态: FacilityState.prefault('整洁'),
  风格: z.string(),
  描述: z.string(),
  价格折合铜币: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
});
...
export const Schema = z.object({
  ...
  酒馆: z.object({
    ...
    客房: z.record(z.string().describe('房间名'), z.object({
      ...
      住客: z.string().prefault('无'),
      清洁状态: z.string().prefault(''),
      清洁原因: z.string().prefault(''),
      舒适描述: z.string().prefault(''),
      私密描述: z.string().prefault(''),
      价格描述: z.string().prefault(''),
  ...
  人物羁绊: z.record(z.string().describe('配角名'), z.object({
    ...
    所在位置: z.string(),
    一句话穿着: z.string(),
    生命: StatBar.prefault({ 当前值: 100, 上限: 100 }),
    精力: StatBar.prefault({ 当前值: 100, 上限: 100 }),
    膀胱: StatBar.prefault({ 当前值: 0, 上限: 100 }),
    个人资金: MoneyBucket.prefault({ 铜币: 0, 银币: 0, 金币: 0, 铂金币: 0, 秘银币: 0, 折算合计铜币: 0 }),
    收入: z.object({
      职业: z.string(),
      日收入折合铜币: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
      结算方式: z.string(),
      备注: z.string(),
    }).prefault({ 职业: '', 日收入折合铜币: 0, 结算方式: '', 备注: '' }),
    备注: z.string(),
  })).prefault({}),
  农田与酒窖: z.object({
    农田: z.record(z.string().describe('田畦名'), z.object({
      ...
      播种日: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
      成熟日: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
      预计产出: z.string(),
      批次标签: z.array(z.string()).prefault([]),
    })).prefault({}),
    酒窖桶: z.record(z.string().describe('酒桶名'), z.object({
      状态: BrewState,
      内容物: z.string(),
      类型: z.string(),
      酿造开始日: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
      收获日: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
      预计产出: z.string(),
    })).prefault({}),
  }),
  布草库存: z.record(z.string().describe('布草类型'), z.object({
    总数: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
    干净可用: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
    脏污待洗: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
    晾晒中: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
  })).prefault({}),
  晾晒: z.object({
    晾晒中: z.record(z.string().describe('晾晒批次'), z.object({
      晾晒物品: z.string().prefault(''),
      晾晒位置: z.string().prefault(''),
      来源: z.string().prefault(''),
      弄脏原因: z.string().prefault(''),
      晾晒开始日: z.coerce.number().prefault(0),
      晾晒开始时间: ClockTime,
      预计干燥日: z.coerce.number().prefault(0),
      干燥状态: z.enum(['湿透', '半干', '基本干了', '已干可收']).prefault('湿透'),
      备注: z.string().prefault(''),
    })).prefault({}),
  }).prefault({}),
  厩舍: z.object({
    状态: FacilityState.prefault('停用'),
    风格: z.string().prefault(''),
    描述: z.string().prefault(''),
    容量: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
    当前载具数: z.coerce.number().transform(value => Math.max(0, Math.floor(value))).prefault(0),
    载具: z.record(z.string().describe('载具名'), z.object({
      类型: z.string(),
      品种: z.string(),
      所属: z.string(),
      健康状况: z.string(),
    ...
    饲料储备: z.record(z.string().describe('饲料名'), StorageItem).prefault({}),
  }).prefault({}),
  禽畜圈养: z.object({
    圈舍状态: FacilityState.prefault('停用'),
    圈舍风格: z.string().prefault(''),
    圈舍描述: z.string().prefault(''),
    禽畜: z.record(z.string().describe('禽畜名'), z.object({
      类型: z.string(),
      品种: z.string(),
      数量: z.coerce.number().transform(value => Math.max(0, Math.floor(value))),
      成长阶段: z.enum(['幼崽', '成长期', '成年', '老年']),
      产出物: z.string(),
      产出周期: z.string(),
      上次产出日: z.coerce.number(),
      饲料需求: z.string(),
      健康状况: z.string(),
      备注: z.string(),
    })).prefault({}),
