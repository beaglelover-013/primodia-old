...
type MobileShelf = '食材' | '调料' | '成品' | '酒水' | '杂物';
...
const mobileShelf = ref<MobileShelf>('食材');
...
const finishedFoods = computed(() => game.inventory.filter(item => item.category === '成品'));
const drinks = computed(() => game.inventory.filter(item => item.category === '酒水'));
const mobileShelves = computed(() => [
  { key: '食材' as const, label: '食材', count: ingredients.value.length, items: ingredients.value },
  { key: '调料' as const, label: '调料', count: seasonings.value.length, items: seasonings.value },
  { key: '成品' as const, label: '成品', count: finishedFoods.value.length, items: finishedFoods.value },
  { key: '酒水' as const, label: '酒水', count: drinks.value.length, items: drinks.value },
  { key: '杂物' as const, label: '杂物', count: others.value.length, items: others.value },
]);
const activeMobileItems = computed(() => mobileShelves.value.find(shelf => shelf.key === mobileShelf.value)?.items ?? []);
  ...
  <section id="page-kitchen" class="page pm-paper kitchen-page">
    ...
    <div class="pm-paper-body kitchen-layout">
      ...
      <div class="kitchen-main">
        ...
        <section class="mobile-shelf">
          <div class="mobile-shelf-tabs">
            <button
              v-for="shelf in mobileShelves"
              :key="shelf.key"
              type="button"
              :class="{ active: mobileShelf === shelf.key }"
              @click="mobileShelf = shelf.key"
            >
          ...
          <div v-if="activeMobileItems.length" class="mobile-item-list">
            <article
              v-for="item in activeMobileItems"
              :key="item.id"
            ...
            >
              <div>
                <strong>{{ item.name }}</strong>
                <small>{{ item.category }} · ×{{ item.qty }}{{ stockUnit(item) }}</small>
              </div>
              <span v-if="portionText(item)" class="mobile-portion">{{ portionText(item) }}</span>
              <button type="button">入锅</button>
            </article>
          </div>
          <div v-else class="pm-empty compact">这一类暂时没有库存。</div>
        </section>
...
@media (max-width: 760px) {
  ...
  .mobile-shelf-tabs button.active {
    color: var(--pm-ink);
    border-color: rgba(110, 80, 34, 0.58);
    background: var(--pm-grad-gold);
  }
  .mobile-shelf-tabs small {
    color: var(--pm-ink-dim);
  ...
  .mobile-item-row strong,
  .mobile-item-row small {
    display: block;
    overflow: hidden;
  ...
  .pot-station {
    ...
    box-sizing: border-box;
    background:
      linear-gradient(180deg, rgba(255, 247, 224, 0.96), rgba(218, 188, 130, 0.9));
  }
  .pot-head {
    align-items: center;
    flex-wrap: nowrap;
  ...
  .pot-station .bench-actions {
    display: grid;
    grid-template-columns: repeat(5, minmax(0, 1fr));
    gap: 4px;
  }
  .pot-station .bench-actions .pm-btn {
    min-height: 29px;
    padding: 4px 3px;
    font-size: calc(10.5px * var(--pm-text-scale));
  }
  .pot-station .bench-actions .pm-btn :deep(.pm-icon) {
    display: none;
  }
}
